...
def LoginView_NEW_Custom(request):
    msg=''
    if request.POST:
        email = request.POST.get('email')
        password = request.POST.get('password')
    ...
    return render(request, 'newform/registration/login.html',{'msg' : msg})
	
	
def RegisterView_NEW_Custom(request):
    msg=''
    if request.POST:
        email = request.POST.get('email')
        password = request.POST.get('password')
        ...
        print( "msg = ")
        print (msg)
    return render(request, 'newform/registration/register.html',{'msg':msg})
...
# def new home custom (01-06-2018)

class HomeView_NEW_Custom(TemplateView):
    template_name = 'newform/app/home.html'

    def get_context_data(self, **kwargs):
        ctx = super(HomeView_NEW_Custom, self).get_context_data(**kwargs)
        for x in MembershipType.objects.all():
            ctx[x.name] = x
        # print('ctx:', ctx)
        return ctx
